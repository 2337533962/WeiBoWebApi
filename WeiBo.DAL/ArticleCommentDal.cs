...
{
    /// <summary>
    /// ArticleComment数据访问对象
    /// </summary>
    public partial class ArticleCommentDal
    {
        /// <summary>
        /// 实例化ArticleComment数据访问对象
        /// </summary>
        ...
        /// <summary>
        /// 查询得到ArticleComment表中所有信息
        /// </summary>
        /// <returns>查询到的所有ArticleComment数据模型对象集合</returns>
        public List<ArticleComment> GetAllModel()
        {
            //创建存储查找到的ArticleComment表中信息集合
            List<ArticleComment> list = new List<ArticleComment>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Select uid,articleId,commentId,content,contentTime,fabulous From ArticleComment;"))
            {
                //判断是否查询到了数据
        ...
        /// <summary>
        /// 将传入的ArticleComment数据模型对象数据存入数据库，并将自动编号值存入，传入ArticleComment数据模型对象中
        /// </summary>
        /// <param name="articleComment">要进行添加到数据库的ArticleComment数据模型对象</param>
        ...
        public bool Add(ArticleComment articleComment)
        {
            ...
            //进行插入操作并返回自动编号结果
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Insert Into ArticleComment(uid,articleId,content,contentTime,fabulous) OutPut Inserted.commentId Values(@uid,@articleId,@content,@contentTime,@fabulous);", sqlParameters))
            {
                //判断是否获取到数据
                if (sqlReader.HasRows)
                {
        ...
        /// <summary>
        /// 根据主键获取一条记录返回一个ArticleComment数据模型对象
        /// </summary>
        /// <param name="commentId">commentId</param>
        /// <returns>如果查找到此记录就返回ArticleComment数据模型对象，否则返回null</returns>
        public ArticleComment GetModel(int commentId)
        {
            ...
            //执行一条查找SQL命令
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Select Top 1 uid,articleId,commentId,content,contentTime,fabulous From ArticleComment Where commentId = @commentId;", sqlParameters))
            {
                //判断是否查找到数据
                if (sqlReader.HasRows)
                {
        ...
        public bool Delete(int commentId)
        {
            ...
            //执行一条按照commentId删除记录语句，并返回是否删除成功
            return SqlDataBase.ExecuteNonQuery("Delete From ArticleComment Where commentId = @commentId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此主键对应的记录
        /// </summary>
        ...
        public bool Exists(int commentId)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)SqlDataBase.ExecuteScalar("Select Count(*) From ArticleComment Where commentId = @commentId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Update(ArticleComment articleComment)
        {
            ...
            //执行更新语句，并返回是否更新完成
            return SqlDataBase.ExecuteNonQuery("Update ArticleComment Set uid = @uid,articleId = @articleId,content = @content,contentTime = @contentTime,fabulous = @fabulous Where commentId = @commentId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此记录
        /// </summary>
        /// <param name="articleComment">验证的ArticleComment数据模型对象</param>
        ...
        public bool Exists(ArticleComment articleComment)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)SqlDataBase.ExecuteScalar("Select Count(*) From ArticleComment Where uid = @uid And articleId = @articleId And commentId = @commentId And content = @content And contentTime = @contentTime And fabulous = @fabulous;", sqlParameters) > 0;
        }
        /// <summary>
        ...
        /// <param name="sqlParameters">所需SQL参数对象数组</param>
        /// <returns>查询到的ArticleComment数据模型对象集合</returns>
