...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class ArticleTypeDal
    {
        ...
        /// 查询得到作品类型表表中所有信息
        /// </summary>
        /// <returns>查询到的所有作品类型表数据模型对象集合</returns>
        public List<ArticleType> GetAllModel()
        {
            //创建存储查找到的作品类型表表中信息集合
            List<ArticleType> list = new List<ArticleType>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select typeId,type From ArticleType;"))
            {
                //判断是否查询到了数据
        ...
        /// <summary>
        /// 将传入的作品类型表数据模型对象数据存入数据库，并将自动编号值存入，传入作品类型表数据模型对象中
        /// </summary>
        /// <param name="articleType">要进行添加到数据库的作品类型表数据模型对象</param>
        ...
        public bool Add(ArticleType articleType)
        {
            ...
            //进行插入操作并返回自动编号结果
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Insert Into ArticleType(type) OutPut Inserted.typeId Values(@type);", sqlParameters))
            {
                //判断是否获取到数据
                if (sqlReader.HasRows)
                {
        ...
        /// <summary>
        /// 根据主键获取一条记录返回一个作品类型表数据模型对象
        /// </summary>
        /// <param name="typeId">类型Id</param>
        /// <returns>如果查找到此记录就返回作品类型表数据模型对象，否则返回null</returns>
        public ArticleType GetModel(int typeId)
        {
            ...
            //执行一条查找SQL命令
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select Top 1 typeId,type From ArticleType Where typeId = @typeId;", sqlParameters))
            {
                //判断是否查找到数据
                if (sqlReader.HasRows)
                {
        ...
        public bool Delete(int typeId)
        {
            ...
            //执行一条按照类型Id删除记录语句，并返回是否删除成功
            return DBHelper.ExecuteNonQuery("Delete From ArticleType Where typeId = @typeId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此主键对应的记录
        /// </summary>
        ...
        public bool Exists(int typeId)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From ArticleType Where typeId = @typeId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        /// <param name="articleType">作品类型表</param>
        ...
        public bool Update(ArticleType articleType)
        {
            ...
            //执行更新语句，并返回是否更新完成
            return DBHelper.ExecuteNonQuery("Update ArticleType Set type = @type Where typeId = @typeId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此记录
        /// </summary>
        /// <param name="articleType">验证的作品类型表数据模型对象</param>
        ...
        public bool Exists(ArticleType articleType)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From ArticleType Where typeId = @typeId And type = @type;", sqlParameters) > 0;
        }
        /// <summary>
        ...
        /// <param name="sqlParameters">所需SQL参数对象数组</param>
        /// <returns>查询到的作品类型表数据模型对象集合</returns>
