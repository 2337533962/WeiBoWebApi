...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class HistoryDal
    {
        ...
        {
            //创建存储查找到的History表中信息集合
            List<History> list = new List<History>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Select uid,articleId From History;"))
            {
                //判断是否查询到了数据
        ...
        public bool Add(History history)
        {
            ...
            //进行插入操作并返回结果
            return SqlDataBase.ExecuteNonQuery("Insert Into History(uid,articleId) Values(@uid,@articleId);", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Exists(History history)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)SqlDataBase.ExecuteScalar("Select Count(*) From History Where uid = @uid And articleId = @articleId;", sqlParameters) > 0;
        }
        /// <summary>
