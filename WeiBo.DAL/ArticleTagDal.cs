...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class ArticleTagDal
    {
        ...
        public List<ArticleTag> GetAllModel()
        {
            ...
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select tagId,tag From ArticleTag;"))
            {
                //判断是否查询到了数据
        ...
        public bool Add(ArticleTag articleTag)
        {
            ...
            //进行插入操作并返回自动编号结果
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Insert Into ArticleTag(tag) OutPut Inserted.tagId Values(@tag);", sqlParameters))
            {
                //判断是否获取到数据
                if (sqlReader.HasRows)
                {
        ...
        public ArticleTag GetModel(int tagId)
        {
            ...
            //执行一条查找SQL命令
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select Top 1 tagId,tag From ArticleTag Where tagId = @tagId;", sqlParameters))
            {
                //判断是否查找到数据
                if (sqlReader.HasRows)
                {
        ...
        public bool Delete(int tagId)
        {
            ...
            //执行一条按照标签Id删除记录语句，并返回是否删除成功
            return DBHelper.ExecuteNonQuery("Delete From ArticleTag Where tagId = @tagId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此主键对应的记录
        /// </summary>
        ...
        public bool Exists(int tagId)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From ArticleTag Where tagId = @tagId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Update(ArticleTag articleTag)
        {
            ...
            //执行更新语句，并返回是否更新完成
            return DBHelper.ExecuteNonQuery("Update ArticleTag Set tag = @tag Where tagId = @tagId;", sqlParameters) > 0;
        }
        /// <summary>
        /// 判断是否有此记录
        /// </summary>
        ...
        public bool Exists(ArticleTag articleTag)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From ArticleTag Where tagId = @tagId And tag = @tag;", sqlParameters) > 0;
        }
        /// <summary>
