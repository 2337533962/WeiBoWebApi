...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class AttentionInfoDal
    {
        ...
        {
            //创建存储查找到的AttentionInfo表中信息集合
            List<AttentionInfo> list = new List<AttentionInfo>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Select followId,followToId From AttentionInfo;"))
            {
                //判断是否查询到了数据
        ...
        public bool Add(AttentionInfo attentionInfo)
        {
            ...
            //进行插入操作并返回结果
            return SqlDataBase.ExecuteNonQuery("Insert Into AttentionInfo(followId,followToId) Values(@followId,@followToId);", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Exists(AttentionInfo attentionInfo)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)SqlDataBase.ExecuteScalar("Select Count(*) From AttentionInfo Where followId = @followId And followToId = @followToId;", sqlParameters) > 0;
        }
        /// <summary>
