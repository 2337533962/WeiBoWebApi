...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class ArticleImgDal
    {
        ...
        /// 查询得到作品图片表表中所有信息
        /// </summary>
        /// <returns>查询到的所有作品图片表数据模型对象集合</returns>
        public List<ArticleImg> GetAllModel()
        {
            //创建存储查找到的作品图片表表中信息集合
            List<ArticleImg> list = new List<ArticleImg>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select articleId,imgUrl From ArticleImg;"))
            {
                //判断是否查询到了数据
        ...
        /// <summary>
        /// 将传入的作品图片表数据模型对象数据存入数据库，并将自动编号值存入，传入作品图片表数据模型对象中
        /// </summary>
        /// <param name="articleImg">要进行添加到数据库的作品图片表数据模型对象</param>
        ...
        public bool Add(ArticleImg articleImg)
        {
            ...
            //进行插入操作并返回结果
            return DBHelper.ExecuteNonQuery("Insert Into ArticleImg(articleId,imgUrl) Values(@articleId,@imgUrl);", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        /// <param name="articleImg">作品图片表</param>
        ...
        /// 判断是否有此记录
        /// </summary>
        /// <param name="articleImg">验证的作品图片表数据模型对象</param>
        ...
        public bool Exists(ArticleImg articleImg)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From ArticleImg Where articleId = @articleId And imgUrl = @imgUrl;", sqlParameters) > 0;
        }
        /// <summary>
        ...
        /// <param name="sqlParameters">所需SQL参数对象数组</param>
        /// <returns>查询到的作品图片表数据模型对象集合</returns>
