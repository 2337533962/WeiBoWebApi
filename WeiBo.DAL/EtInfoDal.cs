...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class EtInfoDal
    {
        ...
        public List<EtInfo> GetAllModel()
        {
            ...
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = DBHelper.ExecuteReader("Select et,etTo,etTime From EtInfo;"))
            {
                //判断是否查询到了数据
        ...
        public bool Add(EtInfo etInfo)
        {
            ...
            //进行插入操作并返回结果
            return DBHelper.ExecuteNonQuery("Insert Into EtInfo(et,etTo,etTime) Values(@et,@etTo,@etTime);", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Exists(EtInfo etInfo)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)DBHelper.ExecuteScalar("Select Count(*) From EtInfo Where et = @et And etTo = @etTo And etTime = @etTime;", sqlParameters) > 0;
        }
        /// <summary>
