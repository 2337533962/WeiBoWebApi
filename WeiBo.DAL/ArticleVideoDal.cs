...
namespace WeiBoWebApi.DAL
{
    ...
    public partial class ArticleVideoDal
    {
        ...
        {
            //创建存储查找到的ArticleVideo表中信息集合
            List<ArticleVideo> list = new List<ArticleVideo>();
            //使用查询语句查询出所有信息
            using (SqlDataReader sqlReader = SqlDataBase.ExecuteReader("Select articleId,videoUrl From ArticleVideo;"))
            {
                //判断是否查询到了数据
        ...
        public bool Add(ArticleVideo articleVideo)
        {
            ...
            //进行插入操作并返回结果
            return SqlDataBase.ExecuteNonQuery("Insert Into ArticleVideo(articleId,videoUrl) Values(@articleId,@videoUrl);", sqlParameters) > 0;
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        ...
        public bool Exists(ArticleVideo articleVideo)
        {
            ...
            //执行查询语句，并返回是否有此记录
            return (int)SqlDataBase.ExecuteScalar("Select Count(*) From ArticleVideo Where articleId = @articleId And videoUrl = @videoUrl;", sqlParameters) > 0;
        }
        /// <summary>
