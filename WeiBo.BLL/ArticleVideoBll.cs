...
namespace WeiBoWebApi.BLL
{
    ...
    /// ArticleVideo业务逻辑对象
    /// </summary>
    public partial class ArticleVideoBll
    {
        /// <summary>
        /// ArticleVideo数据访问对象
        /// </summary>
        private readonly ArticleVideoDal _articleVideoDal = new ArticleVideoDal();
        /// <summary>
        /// 实例化ArticleVideo业务逻辑对象
        /// </summary>
        public ArticleVideoBll()
        {
            
        }
        /// <summary>
        /// 查询得到ArticleVideo表中所有信息
        /// </summary>
        /// <returns>查询到的所有ArticleVideo数据模型对象集合</returns>
        public List<ArticleVideo> GetAllModel()
        {
            //调用数据库访问层查询表中所有信息方法并将查询结果返回
            return this._articleVideoDal.GetAllModel();
        }
        /// <summary>
        /// 将传入的ArticleVideo数据模型对象数据存入数据库，并将自动编号值存入，传入ArticleVideo数据模型对象中
        /// </summary>
        /// <param name="articleVideo">要进行添加到数据库的ArticleVideo数据模型对象</param>
        /// <returns>返回是否添加成功，为true添加成功，为false添加失败</returns>
        public bool Add(ArticleVideo articleVideo)
        {
            //调用数据访问层的添加方法并返回是否添加成功
            return this._articleVideoDal.Add(articleVideo);
        }
        /// <summary>
        /// 更新数据
        /// </summary>
        /// <param name="articleVideo">ArticleVideo</param>
        /// <returns>返回是否更新成功，为true成功为false失败</returns>
        public bool Update(ArticleVideo articleVideo)
        {
            //调用数据访问层更新数据方法并将更新结果返回
            return this._articleVideoDal.Update(articleVideo);
        }
        /// <summary>
        /// 判断是否有此记录
        /// </summary>
        /// <param name="articleVideo">验证的ArticleVideo数据模型对象</param>
        /// <returns>返回是否有此记录，为true代表有此记录，为false代表没有此记录</returns>
        public bool Exists(ArticleVideo articleVideo)
        {
            //调用数据访问层的判断是否有此记录方法并返回判断结果
            return this._articleVideoDal.Exists(articleVideo);
        }
        /// <summary>
        ...
        {
            //调用数据访问层方法
            return this._articleVideoDal.Exists(where, sqlParameters);
        }
        /// <summary>
        /// 自定义删除
        /// </summary>
        ...
        public bool Delete(string where, params SqlParameter[] sqlParameters)
        {
            ...
            return this._articleVideoDal.Delete(where, sqlParameters);
        }
        /// <summary>
        /// 自定义查找
        /// </summary>
        ...
        public List<ArticleVideo> GetModelList(string where, params SqlParameter[] sqlParameters)
        {
            ...
            return this._articleVideoDal.GetModelList(where,sqlParameters);
        }
        /// <summary>
        /// 自定义查询出匹配记录有多少条
        /// </summary>
        ...
        public int GetCount(string where, params SqlParameter[] sqlParameters)
        {
            ...
            return this._articleVideoDal.GetCount(where,sqlParameters);
        }
        /// <summary>
        /// 分页获取数据
        /// </summary>
        ...
        /// <param name="sqlParameters">所需SQL参数对象数组</param>
        /// <returns>查询到的ArticleVideo数据模型对象集合</returns>
        public List<ArticleVideo> GetListByPage(string where, string orderby, bool isDesc, int startIndex, int endIndex, params SqlParameter[] sqlParameters)
        {
            ...
            return this._articleVideoDal.GetListByPage(where,orderby,isDesc,startIndex,endIndex,sqlParameters);
        }
        /// <summary>
        /// 分页获取数据并返回总记录条数
        /// </summary>
        ...
        {
            //调用数据访问层分页获取数据方法
            return this._articleVideoDal.GetMinutePage(where, orderby, isDesc, pageIndex, pageItemCount, sqlParameters);
        }
        /// <summary>
        /// 分页获取数据并返回总记录条数
        /// </summary>
        ...
        {
            //调用数据访问层分页获取数据方法
            return this._articleVideoDal.GetMinutePage(where, orderby, isDesc, pageIndex, pageItemCount, out allItmeCount, sqlParameters);
        }
    }
}
